...
public class ActorController {
    ...
    private final ResourceAssembler<Actor, ActorResource> actorResourceAssembler;

    @GetMapping(path = "")
    public ResponseEntity<PagedResources<ActorResource>> retrieveActors(
            @PageableDefault(page = DEFAULT_PAGE_NUMBER, size = DEFAULT_PAGE_SIZE) Pageable pageRequest,
            PagedResourcesAssembler<Actor> pagedResourcesAssembler) {

        Page<Actor> actors = this.dvdRentalService.retrieveActors(pageRequest);
        Link selfLink = new Link(ServletUriComponentsBuilder.fromCurrentRequest().build().toUriString());
        PagedResources<ActorResource> result = pagedResourcesAssembler.toResource(actors, this.actorResourceAssembler, selfLink);
